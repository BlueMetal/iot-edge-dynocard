...
namespace DynoCardWebAPI.Repos
{
    public class DynoCardAnomalyEventRepo : IDynoCardAnomalyEventRepo
    {
        ...
        public List<AnomalyEvent> Get()
        {
            ...
            using (SqlConnection conn = new SqlConnection(this.settings.ConnectionStrings.DynoCardDbConnString))
            {
                ...
                {
                    var results = eventSelect.ExecuteReader();
                    ...
                    if (results.HasRows)
                    {
                        while (results.Read())
                        {
                            ...
                            {
                                CardHeader_ID = ((int)results["CardHeader_ID"]).ToString(),
                                Card_ID = (results["Card_ID"] is DBNull ? 0 : (int)results["Card_ID"]).ToString(),
                                Card_Type = (string)results["Card_Type"],
                                EPOC_DATE = ((int)results["EPOC_DATE"]).ToString(),
                                Event_ID = ((int)results["Event_ID"]).ToString(),
                                Pump_ID = ((int)results["Pump_ID"]).ToString(),
                                Load = ((float)results["Load"]).ToString(),
                                Position = ((float)results["Postion"]).ToString()
                            });
                        }
                    }
                }
