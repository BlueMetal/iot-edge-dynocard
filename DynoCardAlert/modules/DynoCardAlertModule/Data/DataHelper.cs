...
namespace DynoCardAlertModule.Data
{
    public static class DataHelper
    {
        ...
        public static async Task<DynoCard> LookupPartialDynoCard()
        {
            DateTime cutoff = DateTime.Now.Subtract(TimeSpan.FromSeconds(30));
            string cutoffDateString = cutoff.ToString("s");
            
            var sql = new StringBuilder()
            .Append("SELECT h.CH_ID, ")
            .Append("COUNT(d.CD_ID), ")
            .Append("EV_ID, ")
            .Append("CH_COLLECTED, ")
            ...
            .Append($"WHERE h.CH_UPDATE_DATE >= '{cutoffDateString}' ")
            .Append("GROUP BY h.CH_ID ")
            .Append("EV_ID ")
            .Append("CH_COLLECTED ")
            .Append("CH_SCALED_MAX_LOAD ")
            .Append("CH_SCALED_MIN_LOAD ")
            ...
            .Append("CH_STROKE_PERIOD ")
            .Append("CH_CARD_TYPE ")
            .Append("HAVING COUNT(*) <= 100 ")
            .Append("ORDER BY h.CH_ID DESC ");

            DynoCard partialCard = null;
                ...
                using (Sql.SqlCommand cardHistorySelect = new Sql.SqlCommand(sql.ToString(), conn))
                {
                    ...
                    if (results.HasRows)
                    {
                        while (await results.ReadAsync())
                        {
                            if (partialCard == null)
                            {
                                partialCard = new DynoCard()
                                {
                                    Id = results.GetInt32(0),
                                    Timestamp = results.GetDateTime(3),
                                    MaxLoad = (int)results.GetFloat(4),
                                    MinLoad = (int)results.GetFloat(5),
                                    StrokeLength = (int)results.GetFloat(6),
                                    StrokePeriod = (int)results.GetFloat(7),
                                    CardType = results.GetString(8) == "0" ? DynoCardType.Pump : DynoCardType.Surface,
                                    CardPoints = new List<DynoCardPoint>()
                                };

                                partialCard.CardPoints.Add(new DynoCardPoint()
                                {
                                    Position = (int)results.GetFloat(10),
                                    Load = (int)results.GetFloat(11)
                                });
                            }
                            else
                            {
                                //Exit the loop if it's beyond the current ID. We only care about the first one.
                                int nextId = results.GetInt32(0);

                                if(nextId != partialCard.Id)
                                {
                                    break;
                                }
            ...
            return await Task.FromResult(partialCard);
        }
